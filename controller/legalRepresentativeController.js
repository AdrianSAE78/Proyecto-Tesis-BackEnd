...
const { Student, Asistance, Incident } = require('../model/tableRelations');
...
exports.generateQR = async (req, res) => {
    try {
        ...
        if (!estudiante) {
            return res.status(404).json({ message: 'Estudiante no asociado a este representante' });
        }
        ...
        return res.status(500).json({ message: 'Error generando el QR', error: err.message });
    }
};
...
exports.getAsistance = async (req, res) => {
    try {
        ...
        const asistencias = await Asistance.findAll({ where: { id_student: idEst } });
...
exports.getIncidents = async (req, res) => {
    try {
        ...
        const incidencias = await Incident.findAll({ where: { id_student: idEst } });
        return res.json(incidencias);
    } catch (error) {
        console.error('Error obteniendo incidencias:', error);
        return res.status(500).json({ message: 'Error al obtener incidencias', error: error.message });
    }
};
